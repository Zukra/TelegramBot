...
/**
 *  412846761       (My)
 *  Crypto currency @ForTestCurrencyBot
...
$arGeneralChatId = [412846761, 39661790, -1001339615839];
$arOtherChatID = [-1001339615839];
...
//    $test = $currency->getTestCurrency("https://www.bitstamp.net/api/v2/trading-pairs-info");
    ...
    if (!empty($arrDiff)) {
        ...
        $telegramApi->sendMessageToChats($arGeneralChatId, $msg);
        /*
    ...
    /*
        if (!empty($msgQueue[0]) && time() >= $msgQueue[0]["TIME"]) {
            ...
            $telegramApi->sendMessage($arOtherChatID, $elem["MSG"]);
        }
    */
